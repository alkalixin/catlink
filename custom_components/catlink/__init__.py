...
class FeederDevice(Device):
    ...
    def __init__(self, dat: dict, coordinator: DevicesCoordinator):
        ...
        self.apistr = "feederpro" if dat.get("deviceType") == "FEEDER_PRO" else "feeder"
    ...
    @property
    def desiccant_countdown(self):
        return self.detail.get("desiccantCountdown")

    async def update_device_detail(self):
        api = f"token/device/{self.apistr}/detail".strip()
    ...
    def last_log(self):
        ...
        if not log:
            return None
        return f"{datetime.datetime.now().strftime("%Y-%m-%d")} {log.get('time')} {log.get('event')} {log.get('firstSection')} {log.get('secondSection')}".strip()
    ...
    async def update_logs(self):
        api = f"token/device/{self.apistr}/stats/log/top5".strip()
        pms = {
            "deviceId": self.id,
        }
        rsp = None
    ...
    async def food_out(self):
        api = f"token/device/{self.apistr}/foodOut".strip()
        pms = {
            "footOutNum": 5,
            "deviceId": self.id,
        }
    ...
    def hass_sensor(self):
        return {
            ...
            "desiccant_countdown": {
                "icon": "mdi:update",
                "state_attrs": self.desiccant_countdown,
...
class FountainDevice(Device):
    logs: list
    coordinator_logs = None

    @property
    def filter_element_remain(self):
    ...
    @property
    def mode(self):
        sta = self.detail.get("runMode", "")
        return self.modes.get(f"{sta}".strip(), sta)
    ...
    def modes(self):
        return {
            ...
            "INDUCTION_SPRING": "智能涌泉",
        }

    @property
    def mode_attrs(self):
        return {
            "work_mode": self.detail.get("runMode"),
        }
    ...
    def last_log(self):
        ...
        if not log:
            return None
        return f"{datetime.datetime.now().strftime("%Y-%m-%d")} {log.get('time')} {log.get('event')}".strip()
    ...
    def hass_sensor(self):
        return {
            "filter_element_remain": {
                "icon": "mdi:weight-gram",
                "state": self.filter_element_remain,
                "unit": UnitOfTime.DAYS,
            },
            "error": {
                "icon": "mdi:alert-circle",
    ...
    async def select_mode(self, action):
        ...
        eno = rdt.get("returnCode", 0)
        if eno:
            _LOGGER.error("Switch mod failed: %s", [rdt, pms])
            return False
        await self.update_device_detail()
        return rdt
...
class ScooperDevice(Device):
    ...
    def string_to_number(self, s, default_value=None):
        try:
            # 尝试将字符串转换为float
            return float(s)
        except ValueError:
    ...
    def temperature(self):
        return self.string_to_number(self.detail.get("temperature", "0"))

    @property
    def humidity(self):
        return self.string_to_number(self.detail.get("humidity", "0"))

    @property
    def catLitterWeight(self):
        return self.detail.get("catLitterWeight", "0")
...
class CatlinkEntity(CoordinatorEntity):
    ...
    def update(self):
        if hasattr(self._device, self._entity_key):
            ...
            _LOGGER.debug(
                "Entity update: %s",
                [self.entity_id, self._entity_key, self._attr_state],
            )
